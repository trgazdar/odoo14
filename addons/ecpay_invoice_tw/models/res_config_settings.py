...
class EcpayInvocieResConfigSettings(models.TransientModel):
    ...
    ecpay_demo_mode = fields.Boolean(string='測試模式', help='會使用測試電子發票的API網址進行開票', related='company_id.ecpay_demo_mode', readonly=False)
    ...
    auto_invoice = fields.Selection(string='開立電子發票方式', required=True,
                                    selection=[('manual', '手動'), ('automatic', '自動'), ('hand in', '人工填入')], related='company_id.auto_invoice', readonly=False)
