...
class MarketplaceStock(models.Model):
    ...
    def change_product_qty(self):
        for template_obj in self:
            ...
            #different stock.quant record is managed for different location_id
            rec_exist =  self.env['stock.quant'].sudo().search([('product_id','=',template_obj.product_id.id),('product_tmpl_id','=',template_obj.product_temp_id.id),('location_id','=',template_obj.location_id.id)])
            if len(rec_exist) > 1:
                rec_exist[1:].unlink()
            vals = {
